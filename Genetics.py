...
class Chromosome:
 ...
 child_genes = [self.genes[i] if random.random() < 0.5 else partner.genes[i]
for i in range(4)]
...
TARGET)
def main():
 population = [Chromosome(Chromosome.create_chromosome()) for _ in
range(POPULATION_SIZE)]
 ...
 print(f"Values of ['a', 'b', 'c', 'd']: {tuple(int(gene) for gene in
