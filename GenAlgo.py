...
class Chromosome:
    ...
    def crossover(self, other):
        # Two-point crossover
        crossover_points = sorted([random.randint(1, len(self.genes) - 1) for _ in range(2)])
        child_genes = (
                self.genes[:crossover_points[0]] +
                other.genes[crossover_points[0]:crossover_points[1]] +
                self.genes[crossover_points[1]:]
        )
    ...
    def mutate(self):
        ...
        mutated_genes = [gene if random.random() > 0.1 else random.choice(GENES) for idx, gene in enumerate(self.genes)]
